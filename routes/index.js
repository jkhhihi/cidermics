  ...
  if (now >= d)
  {
	...
	*/
	var now = new Date();
	 ...
	 qry="select con_no, con_photo, con_title, if (a.con_upDate > DATE_ADD(now(),INTERVAL -1 DAY) ,'/page_imgs/main_img/new_mark3.svg','') as chkDat from cider.cid_contents a where a.con_release <= '"+_tot+"' order by a.con_no desc limit 0,12";
	 console.log(_tot);
	mysql.select(qry,
	 function (err, data){
		if (err) throw err;
